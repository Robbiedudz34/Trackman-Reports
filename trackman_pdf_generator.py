...
if not trackman_file:  # If no file entered, use the default in the script's directory
    trackman_file = os.path.join(script_dir, "dudz_game_pitchestagged.csv")
        ...
        for csv_col, table_cols in column_mapping.items():
            for table_col in table_cols:
                if isinstance(stats[table_col], list) and stats[table_col]:  # If values were collected
                    if "Max" in table_col:
                        pitch_stats[pitch][table_col] = round(max(stats[table_col], key=abs), 1)  # Fix: Use absolute max
                    elif "Tilt" in table_col:
                        pitch_stats[pitch][table_col] = average_tilt(stats[table_col])
                    else:
                        pitch_stats[pitch][table_col] = round(sum(stats[table_col]) / len(stats[table_col]), 1)  # Fix: Ensure avg calculation
...
def generate_pitch_location_plot(data):
    ...
    color_map, data = get_pitch_color_map_and_types(data)
    data = data.dropna(subset=["PlateLocHeight", "PlateLocSide"])  # Remove rows where PlateLocHeight and PlateLocSide are missing
...
def generate_pitch_movement_plot(data):
    ...
    color_map, data = get_pitch_color_map_and_types(data)
    data = data.dropna(subset=["InducedVertBreak", "HorzBreak"])  # Remove rows where InducedVertBreak and HorzBreak are missing
...
def generate_release_point(data):
    ...
    ax.set_xticks(np.arange(-5, 5.1, 0.1))
...
def generate_trackman_report(hand_abbreviation, formatted_name, df_pitch_summary, pitch_location_path, pitch_movement_path, legend_path, release_point_path, velocity_ridgeline_path, pitch_usage_path, tilt_range_path):
    ...
    buffer.seek(0)
    temp_pdf_path = "temp_trackman_report.pdf"
...
# Create Report
generate_trackman_report(hand_abbreviation, formatted_name, df_pitch_summary, pitch_location_path, pitch_movement_path, legend_path, release_point_path, velocity_ridgeline_path, pitch_usage_path, tilt_range_path)